...
cluster_ts <- function(x){
  UseMethod("cluster_ts", x)
}

cluster_ts.default <- function(x){
  return(x)
}
...
cluster_ids.default <- function(x){
  return(x)
}

#' @title Cluster interactions
#'
...
#'
#' @param df A data.frame of proximity data
...
#'
#' @return A data.frame with an additional column "clusterDyad"  running from clusterOffset + nr of clusters found.
#'
#' @export
cluster_transit.interact <- function(df, clusterOffset=0, by.col="clusterTS", max.r = 10){
  if (nrow(df) == 0){
    warning("Timestamp dataframe is empty!")
    return(df)
  }

  if ( !(by.col %in% names(df)) ){
    warning(paste("data frame has no '",by.col,"' column. Clustering over all badge dyads (!) or run cluster.interaction.ts() first."))
  }

  #create / reset clusterDyad
  df$clusterDyad <- NA

  clusterID <- clusterOffset

  #how many subgroups are there
  subgroups <- unique(df[[by.col]])

  for (i in 1:length(subgroups)) {
    dfs <- subset(df, clusterTS == subgroups[i])
    ...
    while (j < nrow(dfs)){
      ...
      curBID <- dfs[j,"Badge.ID"]
      curOID <- dfs[j,"Other.ID"]
      ...
      if (length(exists)==0){
        ...
        #mid <- max(dfs[is.numeric(dfs$clusterDyad), ])
        #mid<-max(dfs[which(!is.na(dfs$clusterDyad)),"clusterDyad"])
        #print(paste("clusterid",clusterID, "max id", mid))

        clids[[clusterID]] <- c(curBID, curOID)
        dfs[j,"clusterDyad"] <- clusterID
    ...
    #copy clusters of this subgroup to df
    for (i in 1:nrow(dfs)){
      df[df$Rowid == dfs[i, "Rowid"],"clusterDyad"] <- dfs[i,"clusterDyad"]
    }
  } #end for subgroups

  message(paste((clusterID-clusterOffset+1), " dyad clusters over ",nrow(df)," rows created."))
...
#'
#' @param df Dataframe of interaction data.
...
#'
#' @export
cluster_ts.smtrx <- function(df, twl=120, force.cut="none",  clusterOffset=1){
  ...
  #create/reset cluster ids column in df
  df$clusterTS <- NA
  ...
  startS <- df[1,"Timestamp"]
  endS <- startS + twl
  df[1,]$clusterTS <- clusterID

  #df has just one entry
  if (nrow(df) == 1){
    return(df)
  }

  if (force.cut=="ncluster"){
    twl <- floor(nrow(df)/twl)
  }
  ...
  for (i in 2:nrow(df)){
    ...
    if (df[i,"Timestamp"] <= endS & force.cut=="none" ){
      #extend the time window
      endS <- df[i,"Timestamp"] + twl

      #fall into current session window and cut is enforced.
    } else if (df[i,"Timestamp"] <= endS & force.cut=="twl"){
      ...
      clusterID <- clusterID +1
      startS <- df[i,"Timestamp"]
      endS <- startS + twl
      rowcount <- 1
    }

    #assign cluster ID to current row
    df[i,]$clusterTS <- clusterID
  }
  message(paste((clusterID-clusterOffset+1), " timestamp clusters over ",nrow(df)," rows created."))
...
#'
#' @param df Dataframe with Badge.ID column
#' @param clusterOffSet Numeric start of cluster ID number
#'
#' @return Modified dataframe with additional column "clusterIDSeq".
#'
#' @export
cluster_ids.smtrx <- function(df, clusterOffSet=1){


  lastID = df[1,"Badge.ID"]

  #starting number of cluster
  cluster=clusterOffSet

  #assign start
  df$clusterIDSeq <- NULL;
  df[1,"clusterIDSeq"] <- cluster


  for (i in 2:nrow(df)){

    currentID <- df[i, "Badge.ID"]
    ...
    df[i,"clusterIDSeq"] <- cluster
  }

  message(paste((cluster-clusterOffSet+1), " badge clusters over ",nrow(df)," rows created."))
